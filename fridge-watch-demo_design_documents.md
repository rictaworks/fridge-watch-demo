...
5. 今日から**2年超先の日付は誤読として棄却**する
6. 日付が採用できない場合はカテゴリ別デフォルト期限マスタで補完し、UIに「推定」バッジを表示する
7. 抽出文字列をカテゴリキーワード辞書と照合してカテゴリを決定する(未ヒット時は「その他」)
8. OCR完全失敗時は手動入力フォームにフォールバックする
...
| デフォルト賞味期限マスタ | **8件** | カテゴリごとの補完日数(例: 乳製品5日、野菜7日、調味料180日) |
| 日次消費率マスタ | **8件** | カテゴリごとの残量減少率(例: 乳製品20%/日、調味料1%/日) |
| 日付抽出パターンマスタ | **6件** | YYYY.MM.DD / YYYY年MM月DD日 / YY.MM.DD / MM.DD / YYYY-MM-DD / 消費期限接頭パターン |
...
sequenceDiagram
    ...
    OCR-->>BE: 期限候補+抽出テキスト
    alt 日付採用不可
        BE->>DB: カテゴリ別デフォルト期限で補完(is_estimated=true)
    else OCR完全失敗
        BE-->>FE: 手動入力フォームへフォールバック
    end
